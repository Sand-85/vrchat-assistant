...
- `VRC_MONITOR_WORLD_FETCH_COOLDOWN_MS`：**世界名回源失败冷却**（默认 `1800000`=30 分钟）。与 `WORLD_CACHE_TTL_DAYS`（新鲜度阈值）解耦：一次瞬时失败（网络抖动/限流/超时）后，该世界在冷却期内不再被后台重试，既压住「不可见世界每请求重试」的配额浪费，又保留快速自愈。最小 60s，非法值/0/负回落默认。
